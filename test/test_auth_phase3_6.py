...
class TestSignup:
    def test_signup_page_renders(self, client):
        ...
        assert response.status_code == 200
        assert (
            b"Sign" in response.data
            or b"sign" in response.data
...
class TestSettings:
    ...
    def test_settings_telegram_submit(self, client, test_user, db):
        ...
        response = client.post(
            "/settings/telegram",
            data={"telegram_token": "123456789:ABCxyz"},
            follow_redirects=False,
        )
        ...
        token = (
            db.query(UserToken)
            .filter(UserToken.user_id == test_user.id, UserToken.service == "telegram")
            .first()
        )
    ...
    def test_settings_telegram_empty_token(self, client, test_user):
        ...
        response = client.post("/settings/telegram", data={"telegram_token": ""})
...
class TestDashboard:
    ...
    def test_dashboard_shows_service_status(self, client, test_user, db):
        ...
        # Add Gmail token
        token = UserToken(
            user_id=test_user.id, service="gmail", access_token="dummy_token"
        )
        db.add(token)
        db.commit()
...
class TestServiceStatus:
    ...
    def test_service_status_with_both_tokens(self, client, test_user, db):
        ...
        for service in ["gmail", "telegram"]:
            token = UserToken(
                user_id=test_user.id, service=service, access_token="dummy_token"
            )
            db.add(token)
        db.commit()
